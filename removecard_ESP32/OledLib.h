...
void PrintCardonOLED(String ss) ;  //顯示RFID Card Number 在Oled上
void PrintResultonOLED(String ss) ;  //顯示Result在Oled上
//--------------
...
char oledstr[100] ;    //儲存MAC Address 
...
void initOLED()   //啟動OLED顯示器，並檢查是否成功啟動。
{
    ...
    _TPrint(1,16,"ABCDEF1234567890");  //(50, 15) 顯示Hello。 
...
void PrintMAConOLED(String ss)   //顯示MAC Address在Oled上
{
        sprintf(oledstr,"%s",ss.c_str())  ;  //轉換string字串到char array
       //CopyString2Char(ss,&oledstr[0]) ;  //轉換string字串到char array
      _TPrint(1,16,oledstr);  //(50, 15) 顯示訊息。
       Serial.print("MAC on OLED:(") ;
      Serial.print(oledstr) ;
      Serial.print(")\n") ;     
}
...
void PrintResultonOLED(String ss)   //顯示Result在Oled上
{
        sprintf(oledstr,"%s",ss.c_str())  ;  //轉換string字串到char array
       //CopyString2Char(ss,&oledstr[0]) ;  //轉換string字串到char array
      _TPrint(1,31,oledstr);  //(50, 15) 顯示訊息。
       Serial.print("MAC on OLED:(") ;
      Serial.print(oledstr) ;
      Serial.print(")\n") ;     
}
