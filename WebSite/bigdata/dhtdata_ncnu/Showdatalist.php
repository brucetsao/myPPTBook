...
<?php
	...
	$qrystr="select MAC, count(MAC) as cnt ,max(systime) as d FROM ncnuiot.dhtData WHERE 1 group by MAC order by d,MAC"  ;		//將dhtdata的資料找出來
      ...
      <?php 
		  if(count($d00) >0)
		  {
				for($i=count($d00)-1;$i >=0  ;$i=$i-1)
					{
						echo sprintf("<tr><td>%s</td><td>%d</td><td>%s</td></tr>", $d00[$i], $d01[$i], $d02[$i]);
					 }
		 }
      ?>
