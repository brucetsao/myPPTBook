...
<?php
	...
	$qrystr="select MAC as MAC, count(*) as cnt from ncnuiot.OW_curent group by MAC order by MAC " ;		//將dhtdata的資料找出來
      ...
      <?php 
		  if(count($d00) >0)
		  {
				for($i=count($d00)-1;$i >=0  ;$i=$i-1)
					{
						echo sprintf("<tr><td><a href='NCNUdatalistdetail.php?MAC=%s'>%s</td><td>%d</td></tr>", $d00[$i],$d00[$i], $d01[$i]);
					 }
		 }
      ?>
