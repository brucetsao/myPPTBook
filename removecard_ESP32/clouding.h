...
// 定義完整的 HTTP GET 相關參數
String httphead = "http://iot.arduino.org.tw:8888/bigdata/rfid/removecard.php";
String strGet = "/bigdata/rfid/removecard.php"; // 請求行的開頭（未直接使用）
...
/*
 //http://iot.arduino.org.tw:8888/bigdata/rfid/removecard.php?KEY=1938332656&MAC=B8D61A68DAFC
  ...
  應用程式路徑：/bigdata/rfid/removecard.php?KEY=1938332656&MAC=B8D61A68DAFC（伺服器上的 PHP 程式，用來接收並處理資料）。
  參數：?MAC=B8D61A68DAFC，表示傳送的資料MAC Address
  參數：?KEY=1938332656，表示傳送的資料 RFID Card UUID
...
/*
  ...
  */
  ...
  // 判斷是否匹配並允許通過
  if (String(card) == cardNumber && String(result) == "Not Existed") 
  {
    msg="Not Existed" ;    //無此卡號，無法不啟動
    return true;
  }
  if (String(card) == cardNumber && String(result) == "Canceled") 
  {
    msg="Canceled" ;   //卡號存在但未啟動，讓此卡號啟動
    return true;
  }
...
void SendtoClouding() {
  ...
  if (response != "") 
  {
    ...
    if (success) 
    {
      Serial.println("Card Suspended ok");
