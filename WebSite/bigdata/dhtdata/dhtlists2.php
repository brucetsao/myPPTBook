...
// 格式化表格每一行的模板
    $subrow = "<tr><td>%s</td><td>%s</td><td>%4.2f</td><td>%4.2f</td><td>%d</td><td>%s</td><td>%s</td></tr>";

// 格式化超連結的模板，用於查詢特定 MAC 地址的詳細資料
$op1 = "<a href='listforMAC.php?MAC=%s'>Device Detail(詳細每一筆資料)</a>";

// 格式化超連結的模板，用於查詢特定 MAC 地址的詳細資料
$op2 = "<a href='delforMAC.php?MAC=%s'>Delete this Device(刪除這個裝置資料)</a>";
...
<tr bgcolor="#CFC"> <!-- 標題行，帶有背景色 -->
    <td colspan='7'><div align='center'>Temperature & Humidity Sensor(溫溼度感測裝置)</div></td> <!-- 合併六個單元格 -->
            ...
            <tr> <!-- 表格的標題欄 -->
                ...
                <td>Delete the Device(刪除裝置資料)</td> <!-- 顯示細節的連結 -->
            </tr>
            ...
            if (count($d01) > 0) { // 如果有數據
                ...
                for ($i = 0; $i < count($d01); $i++) {
                    ...
                    $op2a = sprintf($op2, $d01[$i]); // 生成每行的詳細資料連結
                    echo sprintf($subrow, $d01[$i], $d02[$i], $d03[$i], $d04[$i], $d05[$i], $op1a, $op2a); 
                    // 使用格式化字串顯示每行的內容
                }
            }
            ?>
