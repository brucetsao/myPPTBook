...
$qrystr = "SELECT MAC, avg(distance) as distance, max(systime) as systime, count(*) as count FROM sonic WHERE 1 group by MAC order by systime desc ;";
...
<head>
    ...
    <title>Ultrasonic HC-SR04 Device List(超音波測距裝置)</title>
        ...
        <table border="1" align="center" cellspacing="1" cellpadding="1"> <!-- 建立表格 -->
            ...
            if (count($d01) > 0) { // 如果有數據
                ...
                for ($i = 0; $i < count($d01); $i++) {
                    ...
                    echo sprintf($subrow, $d01[$i], $d02[$i], $d03[$i], $d05[$i], $op1a); 
                    // 使用格式化字串顯示每行的內容
                }
            }
            ?>
